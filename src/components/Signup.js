  ...
  return (
    <div className='container mx-auto'>
        <div className='p-5 flex flex-col lg:w-1/3'>
            ...
            <div>
                {process.env.REACT_APP_SIGNUP === 'true' ? <button onClick={handleSubmit} className='bg-dark text-semi_light w-full py-3'>Signup</button> : <button onClick={()=> alert('Signup is currently disabled...')} className='bg-dark text-semi_light w-full py-3'><i className='bi bi-lock mr-2'></i>Signup</button>}
            </div>
        </div>
    </div>
  )
